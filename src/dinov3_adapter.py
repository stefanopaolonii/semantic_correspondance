...
DINOV3_REPO_PATH = ROOT_DIR / "models" / "dinov3"
WEIGHTS_DIR = ROOT_DIR / "asset" / "weights" / "dinov3"

if str(DINOV3_REPO_PATH) not in sys.path:
    sys.path.append(str(DINOV3_REPO_PATH))
...
class DINOv3Adapter(BackboneAdapter):
    """
    Adapter for DINOv3 Vision Transformer backbone.
    Extracts dense patch features from DINOv3 models for semantic correspondence.
    """
    
    MODEL_CONFIGS = {
        'vits16': {
            'arch_func': dinov3_vits16,
        ...
        'vits16_ft2': {
            'arch_func': dinov3_vits16,
            'checkpoint': 'trained/dinov3_vits16_ft2.pth',
            'feature_dim': 384,
            'patch_size': 16,
            'num_blocks': 12
        },
        'vits16_ft4': {
            'arch_func': dinov3_vits16,
            'checkpoint': 'trained/dinov3_vits16_ft4.pth',
            'feature_dim': 384,
            'patch_size': 16,
            'num_blocks': 12
        },
        'vits16_ft6': {
            'arch_func': dinov3_vits16,
            'checkpoint': 'trained/dinov3_vits16_ft6.pth',
            'feature_dim': 384,
            'patch_size': 16,
            'num_blocks': 12
        },
        'vitb16_ft2': {
            'arch_func': dinov3_vitb16,
            'checkpoint': 'trained/dinov3_vitb16_ft2.pth',
            'feature_dim': 768,
            'patch_size': 16,
            'num_blocks': 12
        },
        'vitb16_ft4': {
            'arch_func': dinov3_vitb16,
            'checkpoint': 'trained/dinov3_vitb16_ft4.pth',
            'feature_dim': 768,
            'patch_size': 16,
            'num_blocks': 12
        },
        'vitb16_ft6': {
            'arch_func': dinov3_vitb16,
            'checkpoint': 'trained/dinov3_vitb16_ft6.pth',
            'feature_dim': 768,
            'patch_size': 16,
            'num_blocks': 12
        }
    ...
    def __init__(self, model_arch: str, fine_tune_layers: int = 0):
        super().__init__(model_name='dinov3', model_arch=model_arch, fine_tune_layers=fine_tune_layers)
    ...
    def _get_model_config(self, model_arch: str) -> Dict[str, Any]:
        if model_arch not in self.MODEL_CONFIGS:
            ...
            raise ValueError(f"DINOv3 model '{model_arch}' not supported. Available: {available}")
    ...
    def set_fine_tuning(self, num_layers: int):
        ...
        if num_layers > 0:
            ...
            print(f"Unlocked DINOv3 layers from {start_layer} to {total_blocks-1}")
